...
**Carry both ids; never drop the LID.** Resolution is best-effort by construction — the mapping is learned from traffic, so it will sometimes miss, and WhatsApp only officially supports phone → LID, not the reverse. Treating a resolved phone number as the only identity builds a cliff into every consumer for the day numbers stop being available. So `mentions[]` carries `{lid, phone}` with either side nullable, `from_lid` sits beside `from`, and group participants carry `lid` beside `wa_id`. `phone: null` is a normal answer, not an error.
...
`.github/workflows/deploy.yml` is generated by `apps_deploy_workflow` and should stay verbatim, with **one** deliberate deviation: its health-wait retries `500` as well. The app runs one container at a time (two would fight over the WhatsApp device slot), so a deploy has a real gap where the edge answers 500 — and without that, every successful deploy reports failure. Regenerating the file silently removes it.
